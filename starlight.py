...
def show_pdf_advanced(pdf_path):
    st.subheader("📖 微光文档 | 支持缩放/下载 📥", help="✨ 微光助力，备考无忧 ✨")
    try:
...
def login_form():
    ...
    if st.button("登录", use_container_width=False, key="login_btn", help="点击登录系统",
        ...
        logged_in_users[username] = {
            ...
            "expire_time": time.time() + SESSION_EXPIRE_SECONDS  # 2小时后过期
        }
...
def show_document_content():
    ...
    user_session = logged_in_users[username]
    # 校验session_id和过期时间
    if (user_session["session_id"] != st.session_state.session_id or
            user_session["expire_time"] < time.time()):
        st.error(f"❌ 检测到登录状态失效（过期/其他设备登录），请重新登录！")
        # 清理失效状态
        del logged_in_users[username]
        save_login_state(logged_in_users)
        st.session_state.logged_in = False
    ...
    # -------- 可选：会话续期（每次刷新延长有效期） --------
    logged_in_users[username]["expire_time"] = time.time() + SESSION_EXPIRE_SECONDS
    save_login_state(logged_in_users)
