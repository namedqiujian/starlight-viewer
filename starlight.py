...
# ===================== 新增：登录状态管理 =====================
# 初始化全局已登录用户状态（key: 用户名, value: 会话ID）
if 'logged_in_users' not in st.session_state:
    st.session_state.logged_in_users = {}  # 记录已登录的用户-会话映射
...
def login_form():
    ...
    if st.button("登录", use_container_width=False, key="login_btn", help="点击登录系统",
        ...
        # 2. 校验该用户是否已登录
        logged_in_users = st.session_state.logged_in_users
        if username in logged_in_users:
            # 如果已登录，且不是当前会话（防止页面刷新误判）
            if logged_in_users[username] != st.session_state.session_id:
        ...
        st.session_state.logged_in_users[username] = st.session_state.session_id  # 记录登录状态
...
def show_document_content():
    """登录成功后显示文档内容（新增会话唯一性校验）"""
    current_user = st.session_state.current_user
    username = current_user["username"]
    logged_in_users = st.session_state.logged_in_users

    # 新增：校验当前会话是否是该用户的有效登录会话
    if username not in logged_in_users or logged_in_users[username] != st.session_state.session_id:
        st.error(f"❌ 检测到用户名【{username}】已在其他设备登录，当前会话已失效！")
        # 清除当前会话登录状态
        st.session_state.logged_in = False
        st.session_state.current_user = None
        st.rerun()
